    ...
    function TestPKEYMY:testModule()
        for i,v in ipairs(self.genalg ) do
                ...
                assertErrorMsgContains(
                        "public key not support export private key",
                        k1.export, k1, true,true)
                assert(string.len(k:export(true,true))>0)
                
                assert(string.len(k:export(true,true,true))>0)
                assertErrorMsgContains(
                        "public key not support export private key",
                        k1.export,k1,true,true,true)
                ...
                assertErrorMsgContains(
                        "public key not support export private key",
                        k1.export, k1, true,true,true,'secret')
...
''
        ...
        for k,v in pairs(keys) do
                ...
                -- evp_pkey:export ([export_private=false[, raw_key=false[, pem=true[, passphrase]]]])
                
                -- export_private => true, raw_key => false, pem => false
                local export = pri:export(true, false, false)
                local hex = openssl.hex(export) 
                assertEquals(hex:upper(),v[2])
                
                --export pem
                -- export_private => true, raw_key => false, pem => true
                local pem1  = pri:export(true, false)
                assertStrContains(pem1,'-----BEGIN PRIVATE KEY-----')
                assertStrContains(pem1,'-----END PRIVATE KEY-----')
                
                --export raw pem
                -- export_private => true, raw_key => true, pem => true
                ...
                --export pem
                -- export_private => true, raw_key => false, pem => true, pkcs8
                local pem3  = pri:export(true, false, true,'secret')
                assertStrContains(pem3,'-----BEGIN ENCRYPTED PRIVATE KEY-----')
                assertStrContains(pem3,'-----END ENCRYPTED PRIVATE KEY-----')

                --export raw pem
                -- export_private => true, raw_key => true, pem => true, pkcs8
                local pem4  = pri:export(true, true,  true,'secret')
                ...
                -- export_private => true, raw_key => false, pem => false
                local pem3  = pri:export(true, false, false)
                -- export_private => true, raw_key => true, pem => false
                local pem4  = pri:export(true, true,  false)
                assert(pem3==pem4)

                --export pem
                -- export_private => true, raw_key => false, pem => false, pkcs8
                local pem3  = pri:export(true, false, false,'secret')
                local pri1 = pkey.read(pem3,true,'der','secret')
                -- export_private => true, raw_key => true, pem => false, pkcs8
                local pem4  = pri1:export(true, true,  false,'secret')
                assert(openssl.hex(pem4):upper()==v[2])
